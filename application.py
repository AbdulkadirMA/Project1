...
def book(isbn):
    ...
    user_id = db.execute('SELECT userid FROM users WHERE username=:username',{'username':username}).fetchone()
    uid = user_id[0]
    ...
    if request.method == "GET":
        ...
        reviews = db.execute('SELECT rate, comments, users.userid, username FROM booksreview JOIN users ON (users.userid = booksreview.userid) WHERE booksreview.bookid = :bid;', {'bid': bid}).fetchall()
        users = []
    ...
    if request.method=='POST':
        ...
        if db.execute('SELECT * from booksreview where userid=:uid AND bookid=:bid',{"uid":uid, 'bid':bid}).rowcount==1:
            ...
            db.execute('insert into booksreview (userid,bookid,rate,comments) VALUES (:uid,:bid,:rating, :comment)',{'uid':uid, 'bid':bid, 'rating':rating, 'comment':comment})
            db.commit()
...
def api(ISBN):
    ...
    reviews = db.execute("SELECT * FROM booksreview WHERE bookid = :book_id", {"book_id": book.bookid}).fetchall()
    count = 0
    rate = 0
